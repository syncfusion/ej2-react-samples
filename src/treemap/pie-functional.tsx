...
const Pie = () => {
    ...
    const loaded = (args: ILoadedEventArgs): void => {
        let template: Element = document.getElementById(
            args.treemap.element.id + "_Label_Template_Group"
        );
        if (template) {
            for (let i: number = 0; i < template.childElementCount; i++) {
                AccumulationChartRender(
                    (template.childNodes[i].childNodes[0] as Element).id,
                    i
                );
            }
            let count = 0;
    ...
    const tooltipRendering = (args: ITreeMapTooltipRenderEventArgs): void => {
        //tslint:disable-next-line
        if (args.item["groupIndex"] !== 1) {
            args.cancel = true;
        }
    };
    const AccumulationChartRender = (id: string, i: number): void => {
    ...
    const getData = (count: number): Object => {
            ...
            <div className="control-section">
                <div className="col-md-12">
                    <TreeMapComponent
                        ...
                        tooltipRendering={tooltipRendering.bind(this)}
                        load={load.bind(this)}
                        ...
                        leafItemSettings={{
                            ...
                            labelTemplate:
                                '<div style="height:{{:PieHeight}};width:{{:PieWidth}};" id ={{:Id}}></div>',
                        }}
                    >
