...
export class Integration extends SampleBase<{}, {}> {
    ...
    frameSeries(): void {
        ...
        for (let cell of this.selectedCells) {
            if (cell.measure !== '') {
                let columnSeries: string = (this.pivotGridObj.dataSourceSettings.values.length > 1 && this.measureList[cell.measure]) ?
    ...
    heatmapUpdate() {
        if (this.onInit) {
            ...
            this.heatmap = new HeatMap({
                ...
                xAxis: {
                    title: { text: this.pivotGridObj.dataSourceSettings.rows.map(function (args) { return args.caption || args.name; }).join(' ~ ') },
                    labels: this.xLabels,
                    labelRotation: 315
                },
                yAxis: {
                    title: { text: this.pivotGridObj.dataSourceSettings.values.map(function (args) { return args.caption || args.name; }).join(' ~ ') },
                    labels: this.yLabels,
                },
                dataSource: {
        ...
        return (
            <div className='control-pane'>
                <div className='col-lg-12 control-section' style={{ overflow: 'auto' }}>
                    <PivotViewComponent id='PivotView' ref={d => this.pivotGridObj = d} dataSourceSettings={dataSourceSettings} width={'100%'} height={'300'} dataBound={this.onDataBound.bind(this)}
