...
export class SummaryCustomization extends SampleBase<{}, {}> {
    ...
    onChange(args: any): void {
        this.pivotGridObj.setProperties({ dataSourceSettings: { showGrandTotals: true } }, true);
        this.pivotGridObj.setProperties({ dataSourceSettings: { showRowGrandTotals: true } }, true);
        this.pivotGridObj.setProperties({ dataSourceSettings: { showColumnGrandTotals: true } }, true);
        if (args.value === 'Column') {
            this.pivotGridObj.dataSourceSettings.showColumnGrandTotals = false;
        } else if (args.value === 'Row') {
            this.pivotGridObj.dataSourceSettings.showRowGrandTotals = false;
        } else if (args.value === 'Both') {
            this.pivotGridObj.dataSourceSettings.showGrandTotals = false;
        }
    }

    select(args: SelectEventArgs): void {
        for (let i: number = 0; i < this.pivotGridObj.dataSourceSettings.columns.length; i++) {
            if ((this.pivotGridObj.dataSourceSettings.columns[i].name || this.pivotGridObj.dataSourceSettings.columns[i].caption) === (args.itemData as any).Name) {
                this.pivotGridObj.dataSourceSettings.columns[i].showSubTotals = false;
            }
        }
        for (let i: number = 0; i < this.pivotGridObj.dataSourceSettings.rows.length; i++) {
            if ((this.pivotGridObj.dataSourceSettings.rows[i].name || this.pivotGridObj.dataSourceSettings.rows[i].caption) === (args.itemData as any).Name) {
                this.pivotGridObj.dataSourceSettings.rows[i].showSubTotals = false;
            }
        }
    }
    removed(args: RemoveEventArgs): void {
        for (let i: number = 0; i < this.pivotGridObj.dataSourceSettings.columns.length; i++) {
            if ((this.pivotGridObj.dataSourceSettings.columns[i].name || this.pivotGridObj.dataSourceSettings.columns[i].caption) === (args.itemData as any).Name) {
                this.pivotGridObj.dataSourceSettings.columns[i].showSubTotals = true;
            }
        }
        for (let i: number = 0; i < this.pivotGridObj.dataSourceSettings.rows.length; i++) {
            if ((this.pivotGridObj.dataSourceSettings.rows[i].name || this.pivotGridObj.dataSourceSettings.rows[i].caption) === (args.itemData as any).Name) {
                this.pivotGridObj.dataSourceSettings.rows[i].showSubTotals = true;
            }
        }
    }
        ...
        return (
            <div className='control-pane'>
                <div className='col-lg-8 control-section'>
                    <PivotViewComponent id='PivotView' ref={(scope) => { this.pivotGridObj = scope; }} dataSourceSettings={dataSourceSettings} width={'100%'} height={'400'} gridSettings={{ columnWidth: 140 }}>
                    </PivotViewComponent>
                </div>
