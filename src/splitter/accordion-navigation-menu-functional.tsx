...
import { ExpandEventArgs, AccordionComponent, AccordionItemDirective, AccordionItemsDirective } from "@syncfusion/ej2-react-navigations";
import "./accordion-navigation-menu.component.css";

// Splitter sample with accordion integration

const AccordionIntegration = () => {
  useEffect(() => {
      updateSampleSection();
  }, [])
  let items: [
  ...
  const expand = (e: ExpandEventArgs): void => {
  ...
  const expanding = (e: ExpandEventArgs): void => {
  ...
  const accordionElement = () => {
    return (
      <AccordionComponent id="split_pane1" expanding={expanding.bind(this)} expanded={expand.bind(this)}>
          <AccordionItemsDirective>
              <AccordionItemDirective header={"ASP.NET"} content={splitlist1.bind(this)} expanded={true} />
              <AccordionItemDirective header={"ASP.NET MVC"} content={splitlist2.bind(this)} />
              <AccordionItemDirective header={"JavaScript"} content={splitlist3.bind(this)} />
          </AccordionItemsDirective>
      </AccordionComponent>
    );
  };
  ...
  const splitlist1 = () => {
    return (
      <ListViewComponent id="split-list1" dataSource={ListData1} select={onSelect.bind(this)} />
    );
  };

  const splitlist2 = () => {
    return (
      <ListViewComponent id="split-list2" dataSource={ListData2} select={onSelect.bind(this)} />
    );
  };

  const splitlist3 = () => {
    return (
      <ListViewComponent id="split-list3" dataSource={ListData3} select={onSelect.bind(this)} />   
    );
  };
  ...
  return (
    <div id="accordionSplitter" className="control-section">
      <div id="splitter-01" className="splitter-custom">
        <SplitterComponent height="288px" width="100%" ref={splitterInstance}>
          <PanesDirective>
