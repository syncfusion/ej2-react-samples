...
const DashedLine = () => {
    ...
    const legendRender = (args: ILegendRenderEventArgs) => {
    ...
    return (
        <div className="control-pane">
            ...
            <div className="control-section">
                <ChartComponent id="charts" style={{ textAlign: 'center' }} primaryXAxis={{ valueType: 'Double', majorGridLines: { width: 0 }, minimum: 1950, maximum: 2100, interval: 30, labelIntersectAction: Browser.isDevice ? 'None' : 'Trim', labelRotation: Browser.isDevice ? -45 : 0, majorTickLines: {width : 0}, minorTickLines: {width: 0} }} load={load.bind(this)} primaryYAxis={{ maximum: 12e9, interval: 2e9, lineStyle: { width: 0 }, title: 'Population (in Billions)', majorTickLines: { width: 0 }, minorTickLines: { width: 0 } }} chartArea={{ border: { width: 0 }, margin: { bottom: 12 } }} tooltip={{ enable: true, shared: true, showNearestTooltip: true }} legendSettings={{ visible: true }} crosshair={{ enable: false, line: { color: 'rgba(204,214,235,0.25)', width: Browser.isDevice ? 50 : 20}, lineType: 'Vertical' }} width={Browser.isDevice ? '100%' : '75%'} title="Global Population Trends by Age Group (1950–2100)" subTitle='Source: ourworldindata.org' loaded={onChartLoad.bind(this)} axisLabelRender={axisLabelRender.bind(this)} sharedTooltipRender={sharedTooltipRender.bind(this)} legendRender={legendRender.bind(this)}>
