...
export let multiColoredLineData: DataPoint[] = [
...
let colors: string[] = ['red', 'green', '#ff0097', 'crimson', 'blue', 'darkorange', 'deepskyblue', 'mediumvioletred', 'violet', 'peru', 'gray', 'deeppink', 'navy'];
multiColoredLineData = multiColoredLineData.map((value: DataPoint, index: number) => ({
    ...value,
        ...
        return (
            <div className="control-pane">
                ...
                <div className="control-section">
                    <ChartComponent id="charts" style={{ textAlign: 'center' }} primaryXAxis={{ valueType: 'DateTime', labelFormat: 'y', intervalType: 'Years', minimum: new Date(1895, 0, 1), maximum: new Date(2025, 0, 1), edgeLabelPlacement: 'Shift', majorGridLines: { width: 0 } }} primaryYAxis={{ labelFormat: '{value}°F', minimum: 20, maximum: 40, interval: 4, title: 'Mean Temperature (°F)', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, minorTickLines: { width: 0 } }} tooltip={{ enable: true, showNearestTooltip: true, enableAnimation: false, header: '<b>Year: ${point.x}</b>', format: 'Avg Temp : <b>${point.y}</b>', enableHighlight: true }} legendSettings={{ visible: false }} chartArea={{ border: { width: 0 } }} load={this.load.bind(this)} width={Browser.isDevice ? '100%' : '75%'} title="U.S. Historical Average Temperature (1895–2025)" subTitle='Source: ncei.noaa.gov' loaded={this.onChartLoad.bind(this)}>
                ...
                <div id="description">
                    ...
                    <p>
                        More information on the multi colored line series can be found in this <a target="_blank" href="https://ej2.syncfusion.com/react/documentation/chart/chart-types/line#multicolored-line" aria-label="Navigate to the documentation for Multi Colored Line Chart in React Chart component">documentation section</a>.
                    </p>
                </div>
            </div>
        )
