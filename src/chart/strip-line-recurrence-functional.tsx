...
import { ChartComponent, SeriesCollectionDirective, SeriesDirective, Inject, Legend, Category, ColumnSeries, DateTime, Tooltip, ILoadedEventArgs, StripLine, ChartTheme, Border, Highlight } from '@syncfusion/ej2-react-charts';
...
const Striplinerecurrence = () => {
    ...
    const [isVisibleX, setIsVisibleX] = useState<boolean>(true);
    const [isVisibleY, setIsVisibleY] = useState<boolean>(true);
    let chartInstance = useRef<ChartComponent>(null);
    let xcheckElement = useRef<HTMLInputElement>(null);
    let ycheckElement = useRef<HTMLInputElement>(null);
