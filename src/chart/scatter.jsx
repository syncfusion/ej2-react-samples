...
import { ChartComponent, SeriesCollectionDirective, SeriesDirective, Inject, Legend, Category, ScatterSeries, Tooltip, } from '@syncfusion/ej2-react-charts';
...
export class Scatter extends SampleBase {
    render() {
        return (<div className='control-pane'>
            ...
            rangePadding: 'None'
        }} load={this.load.bind(this)} title='Height vs Weight' loaded={this.onChartLoad.bind(this)} tooltip={{ enable: true, format: 'Weight: <b>${point.x} lbs</b> <br/> Height: <b>${point.y}"</b>' }} width={Browser.isDevice ? '100%' : '80%'} chartArea={{ border: { width: 0 } }}>
                        <Inject services={[ScatterSeries, Legend, Tooltip, Category]}/>
                        <SeriesCollectionDirective>
                            <SeriesDirective dataSource={scatterData.getMaleData} width={2} xName='x' yName='y' name='Male' type='Scatter' opacity={0.6} marker={{ visible: false, width: 12, height: 12, shape: 'Circle' }}>
                            </SeriesDirective>
                            <SeriesDirective dataSource={scatterData.getFemaleData} xName='x' yName='y' name='Female' type='Scatter' opacity={0.6} marker={{ visible: false, width: 12, height: 12, shape: 'Diamond' }}>
                            </SeriesDirective>
                        </SeriesCollectionDirective>
                    </ChartComponent>
                </div>
