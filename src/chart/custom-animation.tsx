        ...
        return (
            <div className="control-pane">
                ...
                <div className="control-section">
                    <ChartComponent id="charts" style={{ textAlign: 'center' }} primaryXAxis={{ valueType: 'DateTime', edgeLabelPlacement: 'Shift', majorGridLines: { width: 0 }, labelFormat: 'y' }} load={this.load.bind(this)} primaryYAxis={{ title: 'Yield (In Tons per Hectare)', lineStyle: { width: 0 }, majorTickLines: { width: 0 }, maximum: 8, interval: 2, minorTickLines: { width: 0 } }} chartArea={{ border: { width: 0 }, margin: { bottom: 12 } }} tooltip={{
                        enable: true,
                        header: '<b>Almond Yield - ${point.x}</b>',
                        format: '${series.name}: <b>${point.y}</b>',
                        enableHighlight: true,
                        showNearestTooltip: true
                    }} legendSettings={{ visible: true, enableHighlight: true }} width={Browser.isDevice ? '100%' : '75%'} title="Almond Yield Comparison Across Countries (2010–2022)" loaded={this.onChartLoad.bind(this)}>
