...
export class Pagination extends SampleBase<{}, {}> {
  ...
  private rainfalls: number = 0;
  private moistureLevels: number = 30;
  private breezeSpeeds: number = 5;
  private weather: string = 'Sunny';
  private isFahrenheit: boolean = false;

  public updateChart = (buttonId: string, img: string, tempCount: number, chartDay: string, zoomPos: number, zoomFactor: number, rainfall: number, moistureLevel: number, breezeSpeed: number, weatherCondition: string) => {
    this.image = img;
    ...
    this.chartInstance.duration = 600;
  ...
  public chartMouseClick = (args: IMouseEventArgs): void => {
    if (this.chartInstance) {
      ...
      if (index !== -1 && this.temperatureData[index]) {
        const { celsius, fahrenheit } = this.temperatureData[index];
        ...
        if (args.target === 'celsius' && temperatureElement) {
          ...
          this.chartInstance.series[0].marker.dataLabel.name = 'datalabel';
          ...
          this.chartInstance.series[0].marker.dataLabel.name = 'datalabel1';
        }
      }
    ...
    return (<div className='control-pane' style={{ textAlign: "center" }}>
      ...
      <div className='control-section' style={{ textAlign: "center" }}>
        ...
        <div id="chart-button-container">
          <button id="friday" className="chart-custom-button active" onClick={() => { buttonIndex = 0; this.updateChart('friday', 'sunny_image', 12, 'Friday', 0, 0.175, 0, 30, 5, 'Sunny'); }}>
            <img src="src/chart/images/sunny_image.png" alt="Friday" />
            <div className="day">Friday</div>
            <div className="temp">2°C - 12°C</div>
          </button>
          <button id="saturday" className="chart-custom-button" onClick={() => { buttonIndex = 1; this.updateChart('saturday', 'sunny_image', 16, 'Saturday', 0.206, 0.175, 0, 35, 6, 'Sunny'); }}>
            <img src="src/chart/images/sunny_image.png" alt="Saturday" />
            <div className="day">Saturday</div>
            <div className="temp">6°C - 16°C</div>
          </button>
          <button id="sunday" className="chart-custom-button" onClick={() => { buttonIndex = 2; this.updateChart('sunday', 'cloudy', 18, 'Sunday', 0.413, 0.175, 1, 40, 4, 'Cloudy'); }}>
            <img src="src/chart/images/cloudy.png" alt="Sunday" />
            <div className="day">Sunday</div>
            <div className="temp">15°C - 18°C</div>
          </button>
          <button id="monday" className="chart-custom-button" onClick={() => { buttonIndex = 3; this.updateChart('monday', 'cloudy', 16, 'Monday', 0.620, 0.175, 2, 45, 5, 'Cloudy'); }}>
            <img src="src/chart/images/cloudy.png" alt="Monday" />
            <div className="day">Monday</div>
            <div className="temp">12°C - 16°C</div>
          </button>
          <button id="tuesday" className="chart-custom-button" onClick={() => { buttonIndex = 4; this.updateChart('tuesday', 'rainy', 18, 'Tuesday', 0.827, 0.175, 5, 50, 6, 'Rainy'); }}>
