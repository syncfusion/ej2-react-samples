...
function FamilyTreeDiagram() {
    ...
    const nodeState = useMemo(() => new Map<string, NodeUIState>(), []);
    const originalSize = useMemo(() => new Map<string, { width: number; height: number }>(), []);
    const hoveredIdRef = useRef<string | null>(null);

    const DATA_SOURCE: Person[] = familyTreeData as Person[];
    ...
    const buildRelations = (data: Person[]): Relations => {
    ...
    const RELATIONS: Relations = useMemo(() => buildRelations(DATA_SOURCE), [DATA_SOURCE]);

    const relatedSet = useCallback((personId: string) => {
        ...
        parentUnions.forEach((u) => (RELATIONS.unionOf.get(u) ?? []).forEach((p) => people.add(p)));
    ...
    const getUpdatedTemplate = useCallback((data: Person, UI: NodeUIState): string => {
...
<div class="${containerCls}">
  <div class="person-card">
    ...
    const expandNodeWidth = useCallback((id: string) => {
        const diagram: Diagram = diagramRef.current;
        if (!diagram) return;
    ...
    const restoreNodeSize = useCallback((id: string) => {
        const diagram: Diagram = diagramRef.current;
        if (!diagram) return;
        const node = diagram.getObject(id) as NodeModel | null;
        if (!node) return;
        const orig = originalSize.get(id);
        if (orig) {
    ...
    const renderNode = useCallback((id: string) => {
        const diagram: Diagram = diagramRef.current;
        if (!diagram) return;
        const node = diagram.getObject(id) as NodeModel | null;
        if (!node || (node.data as Person)?.Type === 'Union') return;
        const data = node.data as Person;
        const UI = nodeState.get(id) ?? {};
        node.shape = { type: 'HTML', content: getUpdatedTemplate(data, UI) };
    }, [getUpdatedTemplate, nodeState]);

    const paintConnectors = useCallback((nodeSet: Set<string>) => {
        const diagram: Diagram = diagramRef.current;
        if (!diagram) return;
    ...
    const focusHover = useCallback((id: string) => {
        const diagram: Diagram = diagramRef.current;
        if (!diagram) return;
        ...
        diagram.nodes.forEach((n: NodeModel) => {
            if ((n as any)?.data?.Type === 'Union') return;
            const nodeId = String(n.id);
            const isDimmed = nodeId !== id && !people.has(nodeId);
            const isOpen = nodeId === id;
            nodeState.set(nodeId, { isDimmed, isOpen });
            renderNode(nodeId);
        });
    ...
    //  Events
    const onMouseEnter = useCallback((args: IMouseEventArgs) => {
        const node = args?.actualObject as any;
        if (!node || node.sourceID || node.targetID || (node.data && node.data.Type === 'Union')) return;
        if (node.id) focusHover(String(node.id));
    }, [focusHover]);
    ...
    const dataSourceSettings: DataSourceModel = useMemo(() => {
        return {
            id: 'Id',
            parentId: 'Parents',
    ...
    return (
        <div className="control-pane" style={{ width: "100%", opacity: diagramCreated ? 1 : 0 }}>
            <style>{familytreeCss}</style>
            <div className="control-section family-tree">
                <DiagramComponent
                    id="diagram"
                    ref={diagramRef}
                    width={'100%'}
