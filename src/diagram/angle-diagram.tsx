...
import { Message, Severity } from '@syncfusion/ej2-notifications';
...
function createTextNode(id: string, width: number, height: number, offsetX: number, offsetY: number, content: string, style: NodeModel['style'], constraints: NodeConstraints = NodeConstraints.None): NodeModel {
  return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'Text', content }, style } as NodeModel;
}
function createNativeNode(id: string, width: number, height: number, offsetX: number, offsetY: number, svgContent: string, constraints: NodeConstraints = NodeConstraints.None): NodeModel {
  return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'Native', content: svgContent } } as NodeModel;
}
function createImageNode(id: string, width: number, height: number, offsetX: number, offsetY: number, source: string, rotateAngle?: number, constraints?: NodeConstraints, pivotX?: number, pivotY?: number): NodeModel {
  const node: NodeModel = { id, width, height, offsetX, offsetY, shape: { type: 'Image', source }, style: { fill: 'transparent', strokeColor: 'transparent' } } as NodeModel;
...
function createEllipseNode(id: string, width: number, height: number, offsetX: number, offsetY: number, fill: string, strokeColor: string, strokeWidth: number, constraints: NodeConstraints = NodeConstraints.None): NodeModel {
  return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'Basic', shape: 'Ellipse' }, style: { fill, strokeColor, strokeWidth } } as NodeModel;
}
function createHtmlNode(id: string, width: number, height: number, offsetX: number, offsetY: number, html: string, constraints: NodeConstraints = NodeConstraints.None): NodeModel {
  return { id, width, height, offsetX, offsetY, constraints, shape: { type: 'HTML', content: html } } as NodeModel;
}
function createRectNode(id: string, width: number, height: number, offsetX: number, offsetY: number, fill: string, strokeColor: string, strokeWidth: number, constraints: NodeConstraints = NodeConstraints.None): NodeModel {
  return { id, width, height, offsetX, offsetY, constraints, style: { fill, strokeColor, strokeWidth } } as NodeModel;
}
function createConnectorBezier(id: string, spx: number, spy: number, tpx: number, tpy: number, c1x: number, c1y: number, c2x: number, c2y: number, strokeColor: string, strokeWidth: number, dash: string, opacity: number): ConnectorModel {
  return { id, zIndex: 1, type: 'Bezier', constraints: ConnectorConstraints.None, sourcePoint: { x: spx, y: spy }, targetPoint: { x: tpx, y: tpy }, segments: [{ type: 'Bezier', point1: { x: c1x, y: c1y }, point2: { x: c2x, y: c2y } }], style: { strokeColor, strokeWidth, strokeDashArray: dash, opacity }, sourceDecorator: { shape: 'None' }, targetDecorator: { shape: 'None' } } as ConnectorModel;
}
...
function getEfficiencyHtml(): string {
  return `
...
function calculateEfficiency(): void {
  if (solarData.sunElevation <= 0) {
  ...
  // Gradual noon uplift
  const noonAngle = solarData.currentAngle;
...
function getPerformanceMessage(): { message: string; severity: Severity} {
  if (solarData.efficiency > 60) return { message: 'Excellent Performance', severity: Severity.Success };
  if (solarData.efficiency > 40) return { message: 'Fair Performance', severity: Severity.Warning };
  return { message: 'Poor Performance', severity: Severity.Error };
}
...
export class AngleDiagram extends SampleBase<{}, {}> {
  ...
  render() {
    ...
    return (
      <div className="control-pane diagram-control-pane">
        ...
        <div className="control-section angle-diagram-container" style={{ width: '100%', opacity: 0 }}>
          <DiagramComponent
            id="diagram"
            ...
            created={() => {
              ...
              setTimeout(() => {
                const container = document.querySelector('.angle-diagram-container') as HTMLElement;
            ...
            load={() => {
              if (this.diagramCreated && diagram) {
                setTimeout(() => {
                  initializeLocationDropdown();
                  initializeEfficiencyGauge();
                  initializePerformanceMessage();
        ...
        <div id="description">
          <p>
            This interactive diagram visualizes a solar panel system. The sun&apos;s path is represented by a curved connector, and users
            can adjust the panel&apos;s tilt angle using a numeric input. The system provides immediate feedback through a dynamic efficiency
            gauge and performance messages. Users can also select different locations to observe how regional variations impact overall solar
            efficiency.
          </p>
        </div>
      </div>
