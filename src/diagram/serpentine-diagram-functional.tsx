...
function SerpentineDiagram() {
    const diagramInstance = React.useRef<DiagramComponent>(null);
    const [currentZoom, setCurrentZoom] = React.useState(0.65);

    React.useEffect(() => {
        updateSampleSection();

        const handleResize = () => renderSerpentineLayout();
    ...
    const createDecorator = (color: string, pivotX: number) => ({
    ...
    const renderSerpentineLayout = () => {
        ...
        for (let i = 0; i < nodes.length - 1; i++) {
            ...
            const connector: ConnectorModel = {
                ...
                targetDecorator: createDecorator(color, isRowChange ? 0 : 0.25),
                sourceDecorator: createDecorator(color, 0.25),
    ...
    const handleZoom = (level: number) => {
        ...
        const currentZoomValue = diagram.scrollSettings.currentZoom || 1;
        const zoomFactor = level / currentZoomValue;
...
export default SerpentineDiagram;