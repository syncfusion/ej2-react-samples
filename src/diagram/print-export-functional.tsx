...
function createNode(
    ...
    return {
        ...
        style: { strokeColor, fill: fillColor },
        annotations: [{ content }]
    };
}
...
const exportItems: ItemModel[] = [{ text: "JPG" }, { text: "PNG" }, { text: "SVG" }];
...
function PrintExport() {
    ...
    const checkBoxRef = React.useRef<CheckBoxComponent | null>(null);

    // Handlers
    const onItemClick = React.useCallback((args: ClickEventArgs) => {
    ...
    const onSelectExportFormat = React.useCallback((args: MenuEventArgs) => {
        if (!diagramRef.current || !checkBoxRef.current) return;
    ...
    const contentTemplate = React.useCallback(() => {
        return (
            <DropDownButtonComponent
                items={exportItems}
    ...
    const checkboxTemplate = React.useCallback(() => {
        return (
            <CheckBoxComponent
                id="checkBox"
                checked={false}
                label="Multiple Page"
                ref={(checkBox) => (checkBoxRef.current = checkBox)}
            />
        );
    }, []);
        ...
        <div className="control-pane">
            ...
            <div className="control-section">
                <div style={{ width: "100%" }}>
                    ...
                    {/* Diagram */}
                    <DiagramComponent
                        id="diagram_print_export"
                        ref={(diagram) => (diagramRef.current = diagram)}
                        width={"100%"}
                        height={"580px"}
                        nodes={nodes}
                        ...
                        getConnectorDefaults={(connector: ConnectorModel, _diagram: Diagram) => {
                            connector.style.strokeColor = "#6d6d6d";
                            return connector;
                        }}
                        created={() => diagramRef.current?.fitToPage()}
                    >
