...
const Reorders = () => {
  ...
  const change = (args: ChangeEventArgs): void => {
    ...
    let index: number = treegridObj.current.getColumnIndexByField(columnName);
  ...
  const change2 = (args: ChangeEventArgs): void => {
    ...
    let column: Column = treegridObj.current.columns[toColumnIndex] as Column;
    treegridObj.current.reorderColumns(columnName, column.field);
  };

  const actionComplete = (args: ActionEventArgs): void => {
    if (args.requestType === "reorder") {
      let columnName: string = dropdownObj.current.value as string;
      let index: number = treegridObj.current.getColumnIndexByField(columnName);
      dropdownObj2.current.value = index.toString();
    }
  };
      ...
      <div className="control-section">
        <div className="col-md-9">
          <TreeGridComponent
            ...
            pageSettings={{ pageCount: 4, pageSize: 11 }}
            ref={treegridObj}
