...
function Clipboard() {
    ...
    function onChange(sel: ChangeEventArgs): void {
        ...
        treegridInstance.copyHierarchyMode = mode;
    }
    ...
    function toolbarClick(args: any) {
        if (treegridInstance.getSelectedRecords().length > 0) {
            let withHeader: boolean = false;
            if (args.item.id === 'copyHeader') {
                withHeader = true;
            }
            treegridInstance.copy(withHeader);
        } else {
    ...
    return (<div className='control-pane'>
        <div className='control-section'>
            <div className='col-lg-9'>
                <TreeGridComponent dataSource={sampleData} ref={treegrid => treegridInstance = treegrid} height='350' treeColumnIndex={1} childMapping='subtasks' allowPaging={true} allowSelection={true}
                    selectionSettings={{ type: 'Multiple' }} pageSettings={{ pageSize: 10 }} toolbar={toolbarOptions} toolbarClick={toolbarClick.bind(this)}>
                    <ColumnsDirective>
                    ...
                    <table id='property' title='Properties' className='property-panel-table' style={{ width: '100%' }}>
                        <tr>
                            ...
                            <td style={{ width: '70%', paddingTop: '10px' }}>
                                <div>
                                    <DropDownListComponent width="99px" id="selmode" change={onChange.bind(this)} dataSource={modes} value="Parent" />
                                </div>
                            </td>
                        </tr>
                    </table>
