...
import { TreeGridComponent, TreeGrid, ColumnsDirective, ColumnDirective, Page, Toolbar, ColumnChooser, Inject } from '@syncfusion/ej2-react-treegrid';
...
export class Stacked extends SampleBase<{}, {}> {
  ...
  public gridInstance: TreeGrid;
  public toolbar: any = ['ColumnChooser'];
  public columnChooserSettings: any;
  ...
  public Template = (props: any): any => {
    ...
    return (
      <div>
        {props.columns && props.columns.length ? (<TreeViewComponent fields={fields} cssClass="no-border" showCheckBox={true} nodeClicked={this.nodeCheck} keyPress={this.nodeCheck} ref={(treeview: TreeView) => { this.treeObj = treeview; }} />) : (<div className="no-record-text">No Matches Found</div>)}
      </div>
    );
  }
  ...
  public columnChooserClose = () => {
    (this.gridInstance.grid.columnChooserModule as any).hideDialog();
  }
  ...
  public columnChooserSubmit = () => {
    ...
    var showColumns: any = this.gridInstance.getVisibleColumns().filter((column: any) => { return (column.showInColumnChooser === true); });
    showColumns = showColumns.map((col: any) => { return col.headerText; });
    ...
    return (
      <div className='control-pane'>
        <div className='control-section'>
          <TreeGridComponent ref={(g: any) => this.gridInstance = g} dataSource={stackedData} id="TreeGrid" treeColumnIndex={1} childMapping='subtasks' height='350' allowPaging={true}
            pageSettings={{ pageCount: 5 }} showColumnChooser={true}
            columnChooserSettings={this.columnChooserSettings} toolbar={this.toolbar} clipMode='EllipsisWithTooltip'>
            <ColumnsDirective>
              ...
              { field: 'orderDate', headerText: 'Order Date', width: 110, textAlign: 'Right', format: 'yMd' }]}
              ...
              { field: 'units', headerText: 'Units', width: 80, textAlign: 'Right' },]}
              ...
              { field: 'price', headerText: 'Total Price', width: 115, format: 'C', type: 'number' }]}
