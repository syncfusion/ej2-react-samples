...
const SelectionAPI = () => {
  ...
  let numericObj = useRef<NumericTextBoxComponent>(null);
  let numericObj2 = useRef<NumericTextBoxComponent>(null);

  const btnClick = (): void => {
    let startRow: number = numericObj.current.value;
    let toRow: number = numericObj2.current.value;
    let rows: number[] = [];
    for (
      let i: number = startRow > toRow ? toRow : startRow;
      i <= (startRow > toRow ? startRow : toRow);
      i++
    ) {
      rows.push(i);
    }
    treegridObj.current.selectRows(rows);
  };

  const btnClick2 = (): void => {
    treegridObj.current.clearSelection();
  };
  return (
    <div className="control-pane">
      ...
      <div id="description">
        <p>
          ...
          that you select. Selection can be done through a simple Mouse down or
          Keyboard interaction. To enable selection, set{" "}
        ...
        <ul>
          <li>
            <code>single</code> - Enabled by default. Allows the user to select
            single row/cell at a time.
          </li>
          <li>
        ...
        <ul>
          <li>
            <code>Row</code> - Enabled by default. Enables row selection in Tree
            Grid.
          </li>
          <li>
            <code>Cell</code> - Enables cell selection in Tree Grid.
          </li>
          <li>
            <code>Both</code> - Enables both row and cell selection in Tree
            Grid. Clicking any cell will select both the row and cell
            simultaneously.
          </li>
        </ul>
        <p>
          ...
          set by using the
          <code>selectionSettings-&gt;cellSelectionMode</code> property. They
          are,
        </p>
        <ul>
          <li>
          ...
          <li>
            ...
            end column indexes that includes in between cells of rows within the
            range.
          </li>
        </ul>
        <p>
          To perform the multi-selection, hold <strong>CTRL</strong> key and
          click the desired rows/cells. To select range of rows/cells, hold{" "}
          <strong>SHIFT</strong> key and click the rows/cells.
        </p>
        <p>
        ...
        <p>
          In this demo, enter the values in the Start and To text box to select
          range of rows. And click the Clear Selection button to deselect the
          rows.
        </p>
        <p>
