...
function  formSelectedData ( selectedFiles : FileInfo, proxy: any ): void {
    ...
    liEle.appendChild(createElement('span', {className: 'file-size ', innerHTML: uploadObj.bytesToSize(selectedFiles.size) }));
...
function removeFiles(args: any): void {
    ...
    if (status === '2') {
        uploadObj.remove(filesDetails[filesList.indexOf(args.currentTarget.parentElement)]);
    } else {
								...
								<div id='dropArea' className='dropArea' ref={dropRef}>
									<span id='drop' className='file-name-span drop'> Drop files here or <a href="" id='browse'><u>Browse</u></a> </span>
										<UploaderComponent id='fileUpload' type = 'file' ref = {(scope) => {uploadObj = scope}}
											...
											selected= {onFileSelect.bind(this) }
											progress = {onFileUpload.bind(this) }    // Triggres when upload is in progress
											failure = { onUploadFailed.bind(this) }    // Triggres when upload got failed
