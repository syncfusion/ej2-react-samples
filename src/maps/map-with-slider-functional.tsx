...
function MapSlider() {
    ...
    function sliderChange() {
        if (!isNaN(sliderElement.value[0]) && !isNaN(sliderElement.value[1])) {
            for (let i: number = 0; i < mapInstance.layers[0].shapeSettings.colorMapping.length; i++) {
                if (mapInstance.layers[0].shapeSettings.colorMapping[i].from < sliderElement.value[0] ||
                    mapInstance.layers[0].shapeSettings.colorMapping[i].to > sliderElement.value[1]) {
                    mapInstance.layers[0].shapeSettings.colorMapping[i].color = '#E5E5E5';
                } else {
                    mapInstance.layers[0].shapeSettings.colorMapping[i].color = colorCodes[i];
                }
            }
            sliderVal = sliderElement.value;
            mapInstance.refresh();
        }
    }
    ...
    return (
        <div className='control-pane'>
            ...
            <div className='control-section'>
                <MapsComponent id="maps" load={load.bind(this)} loaded={loaded.bind(this)} ref={m => mapInstance = m}
                    margin={{
                        bottom: 20
                    }}
                    titleSettings={{
            ...
            <SliderComponent id="mapannotation" style={{ width: '300px', marginLeft: '300px' }} type='Range' min={-1.5} max={3.75} step={0.75} value={sliderVal}
                ticks={{ placement: 'After', largeStep: 0.75 }} change={sliderChange.bind(this)} ref={d => sliderElement = d} />
