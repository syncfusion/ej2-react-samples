...
const EventManagement = () => {
  ...
  const allUnplannedEventsTreeViewRef = useRef<TreeViewComponent>(null);
  const CloudSecurityEventTreeViewRef = useRef<TreeViewComponent>(null);
  const AIAutomationEventTreeViewRef = useRef<TreeViewComponent>(null);
  const alertDialogRef = useRef<DialogComponent>(null);
  ...
  const eventsData: Record<string, any>[] = extend([], (dataSource as Record<string, any>).TechnicalEventData, null, true) as Record<string, any>[];
  const unplannedEvent1Data: Record<string, any>[] = extend([], (dataSource as Record<string, any>).CloudSecurityEventData, null, true) as Record<string, any>[];
  const unplannedEvent2Data: Record<string, any>[] = extend([], (dataSource as Record<string, any>).AIAutomationEventData, null, true) as Record<string, any>[];
  const allUnplannedEventsDataRef = useRef<Record<string, any>[]>(
    unplannedEvent1Data.concat(unplannedEvent2Data)
  );
  ...
  const checkRoomCapacity = (Capacity: number, RoomId: number): boolean => {
    const room: Record<string, any> = rooms.find((room) => room.RoomId === RoomId);
    return room && room.RoomCapacity >= Capacity;
  };

  const unPlannedEventsList = useRef(
    unplannedEvents.map((name, index) => ({
      id: index.toString(),
      name: name === '' ? 'All' : name
    }))
  ).current;
  ...
  const getRoomName = (value: any): string => {
    return ((value.resourceData) ?
      value.resourceData[value.resource.textField] :
      value.resourceName);
  };

  const getRoomCapacity = (capacity: string): string => {
    return 'Capacity - ' + capacity;
  };

  const getResourceData = (roomId: number): { [key: string]: Object } => {
    const resources: ResourcesModel = scheduleRef.current!.getResourceCollections().slice(-1)[0];
  ...
  const unplannedEventsUpdatedData = (dataSource: Record<string, any>[], value: string): Record<string, any>[] => {
    return dataSource.filter((data) => data.Title === value);
  };

  const toggleUnplannedEventsElement = (): void => {
    let settingsPanel: HTMLElement = document.querySelector('.unplanned-events-container');
    let toggleButton: HTMLElement = scheduleRef.current!.element.querySelector('.e-show-unplanned-events') || scheduleRef.current!.element.querySelector('.e-hide-unplanned-events');
  ...
  const handlePrintExportSelect = (args: MenuEventArgs): void => {
    switch (args.item.id) {
      case 'print':
  ...
  const onRoomChange = (e: ChangeEventArgs): void => {
    ...
    if (value === 0) {
      scheduleRef.current!.removeResource((previousItemData as any).RoomId, 'Rooms');
      scheduleRef.current!.addResource(rooms, 'Rooms', value);
    } else {
      if (previousItemData.RoomId === 0) {
        let resourceData: Record<string, any>[] = rooms.filter((room) => room.RoomId !== value);
  ...
  const onPopupClose = (args: PopupCloseEventArgs): void => {
    if (args.type === 'Editor') {
      ...
      if (isSaveAction) {
        ...
        const isRoomFiltered: boolean = (scheduleRef.current!.resourceCollection[0].dataSource as Record<string, any>[]).length === 1;
        const isRoomAvailable: boolean = scheduleRef.current!.isSlotAvailable(startTime, endTime, !isRoomFiltered ? roomId - 1 : 0) && startTime.getHours() >= 8 && (endTime.getHours() < 18 || (endTime.getHours() === 18 && endTime.getMinutes() === 0));
        ...
        if (!isRoomAvailable || !isCapacityAvailable) {
          ...
          allUnplannedEventsDataRef.current = allUnplannedEventsDataRef.current.filter(
  ...
  const onTreeDragging = (event: DragAndDropEventArgs): void => {
    ...
    if (scheduleRef.current!.isAdaptive) {
      const classElement: HTMLElement = scheduleRef.current!.element.querySelector('.e-device-hover');
      if (classElement) {
  ...
  const onTreeDragStop = (event: DragAndDropEventArgs): void => {
    ...
    const classElement: Element = scheduleRef.current!.element.querySelector('.e-device-hover');
    if (classElement) {
      classElement.classList.remove('e-device-hover');
    }
    if (!treeviewElement) {
      ...
      if (scheduleElement) {
        let treeviewData: { [key: string]: Object }[] = allUnplannedEventsTreeViewRef.current.fields.dataSource as { [key: string]: Object }[];
        if (event.target.classList.contains('e-work-cells')) {
          ...
          const cellData: CellClickEventArgs = scheduleRef.current!.getCellDetails(event.target);
          ...
          const resourceDetails: ResourceDetails = scheduleRef.current!.getResourcesByIndex(cellData.groupIndex);
          const roomId: number = resourceDetails.resourceData.RoomId;
          const isRoomFiltered: boolean = (scheduleRef.current!.resourceCollection[0].dataSource as Record<string, any>[]).length === 1;
          const isRoomAvailable: boolean = scheduleRef.current!.isSlotAvailable(StartTime, EndTime, !isRoomFiltered ? roomId - 1 : 0);
          const isCapacityAvailable: boolean = checkRoomCapacity(Capacity as number, roomId);
          if (!isRoomAvailable || !isCapacityAvailable) {
  ...
  const onUnplannedEventSelect = (args: ChangeEventArgs): void => {
    ...
    treeviewRefs[selectedUnplannedEventItem].current.fields.dataSource =
      selectedUnplannedEventItem === 0 ? (treeviewRefs[1].current.fields.dataSource as any).concat(treeviewRefs[2].current.fields.dataSource) : unplannedEventsUpdatedData(allUnplannedEventsDataRef.current, unplannedEvents[selectedUnplannedEventItem]);
    handleEmptyDataSourceDisplay(treeviewRefs[selectedUnplannedEventItem].current, treeviewRefs[selectedUnplannedEventItem].current.fields.dataSource as any);
  };
  ...
  const roomValueTemplate = (data: { [key: string]: string }): JSX.Element => {
  ...
  const quickInfoContent = (props: { [key: string]: string }): JSX.Element => {
    let room = '';
  ...
  const roomsDropDown = (): JSX.Element => {
    return (
  ...
  const printAndExport = (): JSX.Element => {
    return (
    ...
    <div className='control-section event-management-control-section'>
      <div className='control-wrapper event-management-wrapper'>
        <ScheduleComponent
          ...
          allowOverlap={false}
          eventSettings={{ dataSource: eventsData, fields: scheduleFields }}
        ...
        >
          ...
          <ToolbarItemsDirective>
            ...
            <ToolbarItemDirective name='Custom' type='Button'
              ...
              click={toggleUnplannedEventsElement} cssClass='toolbar-post-agenda' />
            <ToolbarItemDirective name='Custom' type='Button'
              prefixIcon='e-icons e-print-export'
              template={printAndExport} align='Right' cssClass='toolbar-post-agenda print-export' />
          </ToolbarItemsDirective>
        </ScheduleComponent>
        <div className="unplanned-events-container">
          ...
          <div id="list-container">
            ...
            <TreeViewComponent
              ref={allUnplannedEventsTreeViewRef}
              id="treeview1"
              style={{ display: "" }}
            ...
            />
            <TreeViewComponent
              ref={CloudSecurityEventTreeViewRef}
              id='treeview2'
              style={{ display: "none" }}
              cssClass='event-management-treeview'
              dragArea=".event-management-wrapper"
            ...
            />
            <TreeViewComponent
              ref={AIAutomationEventTreeViewRef}
              id='treeview3'
              style={{ display: "none" }}
              cssClass='event-management-treeview'
              dragArea=".event-management-wrapper"
