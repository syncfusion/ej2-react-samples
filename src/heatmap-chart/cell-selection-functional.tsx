...
function CellSelection() {
    ...
    function cellSelected(args: ISelectedEventArgs): void {
    ...
    function load(args: IChartLoadedEventsArgs): void {
        let selectedTheme: string = location.hash.split('/')[1];
        selectedTheme = selectedTheme ? selectedTheme : 'Material';
        args.chart.theme = (selectedTheme.charAt(0).toUpperCase() + selectedTheme.slice(1)).replace(/-dark/i, "Dark") as ChartTheme;
    }

    function loads(args: ILoadedEventArgs): void {
        let selectedTheme: string = location.hash.split('/')[1];
        selectedTheme = selectedTheme ? selectedTheme : 'Material';
        args.heatmap.theme = (selectedTheme.charAt(0).toUpperCase() + selectedTheme.slice(1)).replace(/-dark/i, "Dark") as HeatMapTheme;
    };

    function Change(args: IChangedEventArgs): void {
    ...
    return (
        <div>
            <div className='col-md-9 control-section'>
                ...
                {/* custom code end */}
                <HeatMapComponent id='heatmap-container' style={{ height: '300px' }} ref={t => heatmap = t}
                    titleSettings={{
                    ...
                    allowSelection={true}
                    showTooltip={true}
                    load={loads.bind(this)}
                    cellSelected={cellSelected.bind(this)}
                    paletteSettings={{
                        palette: [
                            { color: '#3C5E62 ' },
                            { color: '#86C843 ' }
                ...
                <ChartComponent id="container1" style={{ height: '300px' }} ref={t => chart = t}
