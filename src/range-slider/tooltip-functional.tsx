...
import { ChangedEventArgs, SliderComponent, TooltipPlacement, TooltipShowOn } from '@syncfusion/ej2-react-inputs';
...
const Tooltip = () => {
    ...
    const onChange = (args: ChangedEventArgs): void => {
       setTooltip({ ...tooltip, showOn: args.value as TooltipShowOn });
    }    
    // Handling the dropdown list change event to change slider tooltip placement
    const onPlacementChange = (args: ChangedEventArgs): void => {        
        setTooltip({ ...tooltip, placement: args.value as TooltipPlacement });
    }
    // Handler used to reposition the tooltip on page scroll
    const onScroll = (): void => {
        if ((rangeObj as any) .current && (defaultObj as any).current) {
