...
import { SliderComponent, TicksDataModel } from '@syncfusion/ej2-react-inputs';
...
const Orientation = () => {
    ...
    const[ticks,setTicks] = useState<TicksDataModel>({            
        placement: 'Before', 
        largeStep: 20, 
        smallStep: 5, 
        showSmallTicks: true
    ...
    let reverseObj = useRef<SliderComponent>(null);
    // Checkbox change handlers
    const enableDisableTicks = (args: ChangeEventArgs): void => {
        setTicks({ ...ticks, placement: args.checked ? 'Before' : 'None' });
    }
    const enableDisableTooltip = (args: ChangeEventArgs): void => {
        setTooltip({ ...tooltip, isVisible: args.checked });
    }

    const refreshTooltip = (e: any): void => {
        if ((rangeObj as any).current && (defaultObj as any).current && (minRangeObj as any).current && (reverseObj as any).current) {
            ...
            (reverseObj as any).current.refreshTooltip((reverseObj as any).tooltipTarget);
        }
    }
