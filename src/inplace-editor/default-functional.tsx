...
function Default () {
    ...
    let textObj: InPlaceEditorComponent;
    let numericObj: InPlaceEditorComponent;
    let maskObj: InPlaceEditorComponent;
    let editableon: DropDownListComponent
    let editorMode: DropDownListComponent

    let textModel: TextBoxModel = { placeholder: 'Enter employee name' };

    let popupSettings: PopupSettingsModel = { title: 'Enter Employee Name' };

    let numericModel: NumericTextBoxModel = { format: 'c2', value: 100, placeholder: 'Currency format' };

    let maskModel: MaskedTextBoxModel = { mask: '000-000-0000' };
    ...
    // Mapping DropDownList dataSource property
    let editableData: { [key: string]: Object }[] = [
        { 'value': 'Click', 'text': 'Click' }, { 'value': 'DblClick', 'text': 'Double Click' }, { 'value': 'EditIconClick', 'text': 'Edit Icon Click' }
    ];

    // Mapping DropDownList fields property
    let editableFields: FieldSettingsModel = { text: 'text', value: 'value' };

    // Mapping DropDownList value property
    let editableVal: string = 'Click';

    // Change event funtion for DropDownList component   
    function changeEditorMode(e: dropdownsChange): void {
        let mode: string =  editorMode.value as string;
        textObj.mode =  numericObj.mode =  maskObj.mode = mode as RenderMode;
        textObj.dataBind();
        numericObj.dataBind();
        maskObj.dataBind();
    }
    ...
    // Change event funtion for DropDownList component   
    function onEditableOn(e: dropdownsChange): void {
        let editableValue: string =  editableon.value as string;
         textObj.editableOn =  numericObj.editableOn =  maskObj.editableOn = editableValue as EditableType;
         textObj.dataBind();
         numericObj.dataBind();
         maskObj.dataBind();
    }
    ...
    // Change event funtion for CheckBox component
    function onChangeEnable(e: ChangeEventArgs): void {
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            ...
                                            <DropDownListComponent ref={(edit) => {  editorMode = edit }} id='editorMode' className='form-control' dataSource={ dropDownData} fields={ dropDownFields}
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div>
                                            {/* Render the DropDownList Component */}
                                            <DropDownListComponent ref={(edit) => {  editableon = edit }} id='editableon' className='form-control' dataSource={ editableData} fields={ editableFields}
                                                value={ editableVal} width={'90%'} change={ onEditableOn.bind(this)} />
                                        </div>
                                    </td>
                                </tr>
                                <tr>
