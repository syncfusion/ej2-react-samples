...
export class BindUIToDocument extends SampleBase<{}, {}> {
    private placeHolderPrefix = 'placeHolder_';
    ...
    public nameBox : any = document.getElementById('firstName');
    public dateBox : any = document.getElementById('birthDate');
    public addressBox : any = document.getElementById('address');
    public phoneBox : any = document.getElementById('phone');
    public emailBox : any = document.getElementById('email');

    private bindToForm() {
        let contentControlInfos = this.container.documentEditor.exportContentControlData();
    ...
    private bindToDocument () {
        ...
        if (!isNullOrUndefined(bookmarkobj['Name'])) {
            const contentControlData = { title: this.placeHolderPrefix + 'Name', tag: '', value: bookmarkobj['Name'], canDelete: false, canEdit: false, type: 'RichText' };
            data.push(contentControlData);
        }
        if (!isNullOrUndefined(bookmarkobj['DOB'])) {
            const contentControlData = { title: this.placeHolderPrefix + 'DOB', tag: '', value: bookmarkobj['DOB'], canDelete: false, canEdit: false, type: 'Date' };
            data.push(contentControlData);
        }
        if (!isNullOrUndefined(bookmarkobj['Address'])) {
            const contentControlData = { title: this.placeHolderPrefix + 'Address', tag: '', value: bookmarkobj['Address'], canDelete: false, canEdit: false, type: 'RichText' };
            data.push(contentControlData);
        }
        if (!isNullOrUndefined(bookmarkobj['Phone'])) {
            const contentControlData = { title: this.placeHolderPrefix + 'Phone', tag: '', value: bookmarkobj['Phone'], canDelete: false, canEdit: false, type: 'RichText' };
            data.push(contentControlData);
        }
        if (!isNullOrUndefined(bookmarkobj['Email'])) {
            const contentControlData = { title: this.placeHolderPrefix + 'Email', tag: '', value: bookmarkobj['Email'], canDelete: false, canEdit: false, type: 'RichText' };
            data.push(contentControlData);
        }
        ...
        return (<div className='control-pane'>
            <div className="col-lg-9 control-section">
                ...
                <div id="documenteditor_container_body">
                    <DocumentEditorContainerComponent id="container" ref={(scope) => { this.container = scope; }} style={{ 'display': 'block' }}
                       height={'590px'} serviceUrl={this.hostUrl} enableToolbar={true} toolbarItems={this.toolbarOptions} locale='en-US' documentEditorSettings={this.settings} />
                </div>
            </div>
            <div className="col-lg-3 property-section">
                ...
                <div className="property-panel-header">Form UI</div>
                    <div>
                        <TextBoxComponent id = "firstName" placeholder = "Name" floatLabelType = "Auto" />
                    </div>
                    <div style={{marginTop: '20px',marginBottom: '0px'}}>
                        <TextBoxComponent id = "birthDate" placeholder = "Date[DD/MM/YYYY]" floatLabelType = "Auto" />
                    </div>
                    <div style={{marginTop: '20px',marginBottom: '0px'}}>
                        <TextBoxComponent  id = "address" placeholder = "Address" floatLabelType = "Auto" />
                    </div>
                    <div style={{marginTop: '20px',marginBottom: '0px'}}>
                        <TextBoxComponent  id = "phone" placeholder = "Phone" floatLabelType = "Auto" />
                    </div>
                    <div style={{marginTop: '20px',marginBottom: '20px'}}>
                        <TextBoxComponent  id = "email" placeholder = "Email" floatLabelType = "Auto" />
                    </div>
                </div>
                <div className="content-wrapper">
                        <div style={{marginTop: '20px',marginRight: '10px', marginBottom: '10px',marginLeft: '0px'}} >
                            <ButtonComponent  id="BindToDocument" onClick={this.bindToDocument.bind(this)}>
                                Bind Form UI Data To Document
                            </ButtonComponent>
                        </div>
                        <div style={{marginTop: '10px', marginBottom: '0px'}} >
                            <ButtonComponent  id="BindToForm" onClick={this.bindToForm.bind(this)}>
