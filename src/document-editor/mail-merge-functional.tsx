...
function MailMerge() {
    ...
    let toolbarOptions: object = ['New', 'Open', 'Separator', 'Undo',
        'Redo',
        'Separator',
        {
    ...
    let Data: { text: string; category: string; htmlAttributes: { draggable: boolean; }; }[];
    function mergeDocument(): void {
        container.documentEditor.saveAsBlob('Docx').then((blob: Blob) => {
            ...
            fileReader.onload = (): void => {
                ...
                let baseUrl: string = 'https://services.syncfusion.com/react/production/api/documenteditor/MailMerge';
    ...
    function showHideWaitingIndicator(show: boolean) {
        let waitingPopUp: HTMLElement = document.getElementById('waiting-popup');
        let inActiveDiv: HTMLElement = document.getElementById('popup-overlay');
        inActiveDiv.style.display = show ? 'block' : 'none';
        waitingPopUp.style.display = show ? 'block' : 'none';
    }
    function showInsertFielddialog(container): void {
    ...
    function insertField(fieldName: any) {
