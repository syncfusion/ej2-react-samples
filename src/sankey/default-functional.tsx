    ...
    <div className="control-pane">
      ...
      <div className="control-section">
        <SankeyComponent
          ...
          tooltip={{ enable: true, nodeTemplate: '${name}: ${value} TBtu', linkTemplate: Browser.isDevice ? '${start.name}: ${start.out} TBtu → <br/> ${target.name}: ${target.in} TBtu' : '${start.name}: ${start.out} TBtu → ${target.name}: ${target.in} TBtu' }}
        ...
        >
          ...
          <SankeyNodesCollectionDirective>
            {nodes.map((node) => (
              <SankeyNodeDirective key={node.id} id={node.id} offset={node.offset} />
            ))}
          </SankeyNodesCollectionDirective>
