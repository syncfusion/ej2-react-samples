...
const Contextual = () => {
    useEffect(() => {
        updateSampleSection();
        (tableElementRef.current as HTMLTableElement).onclick = (args) => {
            ...
            (imageElementRef.current as HTMLImageElement).classList.remove('e-image-selected');
        }
        (imageElementRef.current as HTMLImageElement).onclick = (e) => {
            e.stopPropagation();
        ...
        (placeholderElementRef.current as HTMLElement).onclick = (args) => {
    ...
    let selectedCell: HTMLElement = null;
    ...
    const tableDropdownOptions: ItemModel[] = [{ text: "Header Row" }, { text: "Banded Rows" }, { text: "Banded Columns" }];
    const borderDropdownOptions: ItemModel[] = [
    ...
    const mergeDropdownOptions: ItemModel[] = [ { text: 'Merge Cells', iconCss: 'e-icons e-merge-cells' }, { text: 'Split Cells', iconCss: 'e-icons e-split-horizontal' } ];
    ...
    const updateSelectedState = (args: string) => {
        if (selectedCell) {
            selectedCell.classList.remove('e-table-selected');
            selectedCell = null;
        }
        (imageElementRef.current as HTMLImageElement).classList[args === 'Image' ? 'add' : 'remove']('e-image-selected');
    }
